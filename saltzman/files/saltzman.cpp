...
int main()
{
    ...
    */
    ...
    //文件路径
    const char* filev = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\V.txt";    //记录单元速度
    const char* filerho = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\rho.txt";    //记录单元密度
    const char* filep = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\P.txt";    //记录单元压力
    const char* filee = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\e.txt";    //记录单元内能
    ...
    {
        //文件名
        const char* plotv = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\V.plt";    //单元速度
        const char* plotrho = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\rho.plt";    //单元密度
        const char* plotp = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\P.plt";    //单元压力
        const char* plote = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\e.plt";    //单元内能
        const char* point = "cell-centered_Lagrangian_Scheme\\saltzman\\output\\mesh.plt";
        ...
        //先画速度
        {
            ...
            for (h=0; h<nx; h++)
            {
                fv<<"\t"<<loc[tl][h][0][0]<<"\t"<<cellv[tl][h][0][0]<<endl;
            }
            fv.close();
        }
        ...
        //再画密度
        {
            ...
            frho<<"ZONE I = "<<nx*ny<<", J = "<<(nx-1)*(ny-1)<<", F=FEPOINT, ET=QUADRILATERAL"<<endl;
            for (h=0; h<nx; h++)
            {
                for (l=0; l<ny; l++)
                {
                    temp = 1 / tau[tl][h][l];
                    frho<<"\t"<<loc[tl][h][l][0]<<"\t"<<loc[tl][h][l][1]<<"\t"<<temp<<endl;
                }
            }
            double ctemp[4];
            for (h=0; h<nx-1; h++)
        ...
        //再画压力
        {
            ...
            for (h=0; h< nx; h++)
            {
                fp<<"\t"<<loc[tl][h][0][0]<<"\t"<<cellP[tl][h][0]<<endl;
            }
            fp.close();
        }
...
void initialmesh()
{
    ...
    for (i=0; i<= nx; i++)
    {
        for (j=0; j<=ny; j++)
        {
            ...
            loc[0][i][j][0] = loc[0][i][j][0] + (0.1-loc[0][i][j][1])* sin(loc[0][i][j][0]*pi);
    ...
    for (h=0; h<nx; h++)
    {
        for (l=0; l<ny; l++)
        {
            //按区域赋值
            {
                tau[0][h][l] = 1/ rho_0;    //比容
                cellv[0][h][l][0] = u_0;    //单元速度
                cellP[0][h][l] = P_0;   //单元压力
            ...
            
            {
                ...
                A[0][h][l] = ( (loc[0][h+1][l][0] - loc[0][h][l][0]) + (loc[0][h+1][l+1][0] - loc[0][h][l+1][0]) ) * hy * 0.5;   //面积
...
{
    int h, l, q, k, k2;
    double alpha1, alpha2, p1, p2, v11, v12, v21, v22, vk, vn;
