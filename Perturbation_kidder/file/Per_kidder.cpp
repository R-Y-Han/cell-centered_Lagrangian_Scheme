...
static int boundary[nx+1][ny+1];   //等于0代表是内点，否则是边界点
...
void anglesolver1(int t_l, int i, int j, double P); //BC顺序为P，vn点的速度
void anglesolver2(int t_l, int i, int j, double P); //BC顺序为vn,P点的速度
void boundarywithP(int t_l, int i, int j, double P);    //边界条件由压力给出的速度
void boundarysolver2(int t_l, int i, int j, double t);  //计算边界点类型为2时的速度
void getboundaryPstar(int t_l, int i, int j);   //计算边界点上边上的压力
void nodalsolver(int t_l, double t); //计算上一时刻网格节点的速度，根据节点性质选择上面三个函数中的一个
...
int main()
{
    ...
    {
        //文件名
        const char* plotr = "cell-centered_Lagrangian_Scheme\\Perturbation_kidder\\output\\r_t.csv";    //单元速度

        const char* point = "cell-centered_Lagrangian_Scheme\\Perturbation_kidder\\output\\mesh.plt";
...

        {
            ...
            fpoint<<"VARIABLES="<<"X"<<","<<"Y,Z"<<endl;
            fpoint<<"ZONE N="<<n<<", E="<<nx*ny<<", F=FEPOINT, ET=QUADRILATERAL"<<endl;
            for (i=0; i<=nx; i++)
            {
                for (j=0; j<=ny; j++)
                {
                    fpoint<<"\t"<<loc[tl][i][j][0]<<"\t"<<loc[tl][i][j][1]<<"\t"<<boundary[i][j]<<endl;
                }
            }
                double ctemp[4];
...
{
    int i, j, h, l, p, q;
    double temp, temp1, temp2, r, theta, rp, tp;

    //先初始化节点相关量
    //计算扰动相关系数
    {
        A_1 = -(xi_1 * pow(r_2,mode-1) - xi_2 * pow(r_1,mode-1));
        A_1 = A_1 / (pow(r_1,-mode-1)*pow(r_2,mode-1) - pow(r_1,mode-1) * pow(r_2,-mode-1));
        A_1 = A_1/ mode;
        
        A_2 = -(xi_1 * pow(r_2,-mode-1) - xi_2 * pow(r_1,-mode-1));
        A_2 = A_2 / (pow(r_1,-mode-1)*pow(r_2,mode-1) - pow(r_1,mode-1) * pow(r_2,-mode-1));
        A_2 = A_2/ mode;
    }
    ...
    for (i=0; i<= nx; i++)
    {
        for (j=0; j<=ny; j++)
        {
            ...
            rp = r + a_0 * mode * ( A_1 / (temp1 * r*r) + A_2 * temp1 ) * cos(mode * theta);
            tp = theta - a_0 * mode * (A_1 /( temp1 * r * r) + A_2 *temp1 ) * sin(mode * theta);

            loc[0][i][j][0] = rp * cos(tp);   //x坐标
            loc[0][i][j][1] = rp * sin(tp);   //y坐标
        ...
        {
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    A[0][h][l] = A[0][h][l] + loc[0][i][j][0] * loc[0][p][q][1];
                    A[0][h][l] = A[0][h][l] - loc[0][i][j][1] * loc[0][p][q][0];
                }
                A[0][h][l] = 0.5 * A[0][h][l];
            }
            {
                ...
                tau[0][h][l] = 1/ temp;    //比容
                cellv[0][h][l][0] = 0;    //单元速度
                cellv[0][h][l][1] = 0;
                cellP[0][h][l] = s * temp * temp;   //单元压力
                gamma[h][l] = gamma_0;  //单元状态方程参数
            }
            
            {
                ...
                c[0][h][l] = gamma[h][l] * cellP[0][h][l]*tau[0][h][l];
                c[0][h][l] = sqrt(c[0][h][l] ); //声速

                //A[0][h][l] = 0.5 * ( (h+1) * hx + r_1 ) * ((h+1) * hx + r_1 ) * hy;
                //A[0][h][l] = A[0][h][l] - 0.5 * ( h * hx + r_1 ) * (h * hx + r_1) * hy;   //面积
...
void anglesolver1(int t_l, int i, int j, double P)    //BC按顺序为P，vn
{
    int h, l, k, q;
...
void anglesolver2(int t_l, int i, int j, double P)  //BC按顺序为vn,P
{
    int h, l, k, q;
    double alpha, temp1, temp2;
    double** A = new double* [3];
    ...
    for (k=0; k< neighbornode[i][j].size(); k++)
    {
        ...
        //计算alpha2 = alpha_{k-1}, p2 = cellP_{k-1}, v2 = cellv_{k-1}
        {
            if (  k == 0 )  //-1邻点对应的网格为虚网格
            {
                ...
                q = neighborcell[i][j][k2];  //(i,j)点的第k-1个邻域的总体编号
        ...
        //计算alpha1 = alpha_k, p1 = cellP_k, v1 = cellv_k
        {
            if ( k == neighbornode[i][j].size() - 1 ) //第k个邻点对应的网格为虚网格
            {
                alpha1 = 0;
                p1 = P;
                v11 = 0;
...
void boundarysolver2(int t_l, int i, int j, double t)
{
    ...
    h = 1 - (t / t_end) * (t / t_end);
    h = sqrt(h);
    if ( i == 0 && j == 0 )    //q为(0,0)的情况
    {
        P = P_1 / (h * h * h * h);
        anglesolver2(t_l,i,j,P);
    }
    else if ( i == 0 && j == ny ) //(0, ny)的情况
    {
        P = P_1 / (h*h*h*h);
...
void nodalsolver(int t_l, double t)
{
    ...
    for (i=0; i<=nx; i++)
    {
        for (j=0; j<=ny; j++)
        {
            ...
            else if(boundary[i][j] == 1)  //边界条件由法向速度给出
            {
                //判断边界是否平行
                double e = 1e-6;
