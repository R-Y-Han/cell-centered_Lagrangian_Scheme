...
void anglesolver1(int t_l, int qm, double P); //(nx,0)点的速度
void anglesolver2(int t_l, int qm, double P); //(nx,ny)点的速度
void boundarywithP(int t_l, int qm, double P);    //边界条件由压力给出的速度
...
int main()
{
    ...
    {
        //文件名
        const char* plotv = "cell-centered_Lagrangian_Scheme\\ICF\\output\\V.plt";    //单元速度
        const char* plotrho = "cell-centered_Lagrangian_Scheme\\ICF\\output\\rho.plt";    //单元密度
        const char* plotp = "cell-centered_Lagrangian_Scheme\\ICF\\output\\P.plt";    //单元压力
        const char* plote = "cell-centered_Lagrangian_Scheme\\ICF\\output\\e.plt";    //单元内能
        const char* point = "cell-centered_Lagrangian_Scheme\\ICF\\output\\mesh.plt";
...
/*
        ...
        //再画密度，由于sod shock是一维问题，所以只要记录x向，下同
        {
            ...
            frho<<"VARIABLES="<<"X"<<", Y"<<","<<"density"<<endl;
            for (h=0; h<2 * nx; h++)
            {
                for (l=0; l<ny; l++)
                {
                    ...
                    frho<<"\t"<<loc[tl][q][0]<<"\t"<<loc[tl][q][1]<<"\t"<<temp<<endl;
                }
        ...
        */
        //再画网格
        {
            ...
            fpoint<<"ZONE N="<<n<<", E="<<2 * nx*ny<<", F=FEPOINT, ET=QUADRILATERAL"<<endl;
            for (q=0; q<n; q++)
            {
...
void initialmesh()
{
    ...
    for (q=0; q<n; q++)
    {
        ...
        if (i<=nx)
        {
            loc[0][q][0] = (i * hx_1) * cos(j * hy);   //x坐标
            loc[0][q][1] = (i * hx_1) * sin(j * hy);   //y坐标
        }
        else{
            loc[0][q][0] = ((i-nx) * hx_2 + r_1) * cos(j * hy);   //x坐标
            loc[0][q][1] = ((i-nx) * hx_2 + r_1) * sin(j * hy);   //y坐标
        }
    ...
    for (h=0; h<2 * nx; h++)
    {
        for (l=0; l<ny; l++)
        {
            
            {
                ...
                if ( h < nx)
                {
                    A[0][h][l] = 0.5 * ( (h+1) * hx_1 ) * ((h+1) * hx_1) * hy;
                    A[0][h][l] = A[0][h][l] - 0.5 * ( h * hx_1 ) * (h * hx_1) * hy;   //面积
                }
                else{
                    A[0][h][l] = 0.5 * ( (h-nx+1) * hx_2 + r_1 ) * ((h-nx+1) * hx_2 + r_1) * hy;
                    A[0][h][l] = A[0][h][l] - 0.5 * ( (h-nx) * hx_2 + r_1 ) * ((h-nx) * hx_2 + r_1) * hy;   //面积
                }
...
void anglesolver1(int t_l, int qm, double P)
{
    int h, l, k, q;
    ...
    //给B赋值
    {
        ...
        B[0] = B[0] + L[t_l][qm][0] * P * Nx[t_l][qm][0];
        ...
        B[1] = B[1] + L[t_l][qm][0] * P * Ny[t_l][qm][0];
...
void anglesolver2(int t_l, int qm, double P)
{
    int h, l, k, q;
    double alpha, temp1, temp2;
    double** A = new double* [3];
    ...
    //给B赋值
    {
        ...
        B[0] = B[0] + L[t_l][qm][1] * cellP[t_l][h][l] * Nx[t_l][qm][1];
        B[0] = B[0] + L[t_l][qm][1] * alpha * temp2 * Nx[t_l][qm][1];
        B[0] = B[0] - L[t_l][qm][1] * P * Nx[t_l][qm][1];

        B[1] = - L[t_l][qm][0] * cellP[t_l][h][l] * Ny[t_l][qm][0];
        B[1] = B[1] + L[t_l][qm][0] * alpha * temp1 * Ny[t_l][qm][0];
        B[1] = B[1] + L[t_l][qm][1] * cellP[t_l][h][l] * Ny[t_l][qm][1];
        B[1] = B[1] + L[t_l][qm][1] * alpha * temp2 * Ny[t_l][qm][1];
        B[1] = B[1] - L[t_l][qm][1] * P * Ny[t_l][qm][1];
...
void boundarywithP(int t_l, int qm, double P)
{
    int h, l, k, k2, q;
    double alpha1, alpha2, p1, p2, v11, v12, v21, v22, vk;
    double** A = new double* [2];
...
void boundarysolver2(int t_l, int qm)
{
    ...
    j = (qm-1)%(ny+1);
    P = 1e14 * (1 + a_0 * sin(2 * j * hy));

    if ( qm == n - ny - 1 )    //q为(nx,0)的情况
    {
        anglesolver1(t_l, qm, P);
    }
    else if ( qm == n-1 ) //(nx, ny)的情况
    {
        anglesolver2(t_l, qm, P);
    }
    else{
        boundarywithP(t_l, qm, P);
    }
